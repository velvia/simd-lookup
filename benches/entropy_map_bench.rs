...
use rand::prelude::*;
use simd_lookup::entropy_map_lookup::{EntropyMapBitpackedLookup, EntropyMapLookup};
...
fn bench_construction_time(c: &mut Criterion) {
    ...
    for &size in &sizes {
        ...
        group.bench_with_input(BenchmarkId::new("rustc_hash", size), &size, |b, _| {
            b.iter(|| {
        ...
        group.bench_with_input(BenchmarkId::new("scalar", size), &size, |b, _| {
            b.iter(|| {
                let lookup_table =
                    simd_lookup::lookup::create_scalar_lookup_table(black_box(&entries));
                let _lookup = simd_lookup::lookup::ScalarLookup::from_table(&lookup_table);
            })
        });

        // Benchmark EntropyMapLookup construction
        group.bench_with_input(BenchmarkId::new("entropy_map_dict", size), &size, |b, _| {
            b.iter(|| {
...
fn bench_large_table_lookup(c: &mut Criterion) {
    ...
    println!(
        "Creating large table: {} entries, {}% density",
        table_size, density
    );
    ...
    let (entropy_bitpacked_lookup, bitpacked_construction_time) =
        EntropyMapBitpackedLookup::new(&entries);

    println!("Construction times:");
    println!(
        "  EntropyMap Dict: {}ms",
        dict_construction_time / 1_000_000
    );
    println!(
        "  EntropyMap Bitpacked: {}ms",
        bitpacked_construction_time / 1_000_000
    );
...
fn bench_memory_and_construction_analysis(c: &mut Criterion) {
    ...
    for &size in &sizes {
        ...
        println!(
            ...
            format!(
                "H:{}KB D:{}KB B:{}KB",
                hash_memory_kb, dict_memory_kb, bitpacked_memory_kb
            )
        );
    }
...
fn bench_density_comparison(c: &mut Criterion) {
    ...
    for &density in &densities {
        ...
        group.bench_with_input(BenchmarkId::new("rustc_hash", density), &density, |b, _| {
            b.iter(|| {
                for &key in &test_keys {
                    black_box(hash_lookup.lookup(black_box(key)));
                }
